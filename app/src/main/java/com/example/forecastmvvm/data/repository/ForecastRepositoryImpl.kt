...
class ForecastRepositoryImpl(
    ...
    private suspend fun initWeatherData(){
        if (isFetcheCurrentNeeded(ZonedDateTime.now().minusHours(1)))
            fetchCurrentWeather()
    }

    private suspend fun fetchCurrentWeather(){
        weatherNetworkDataSource.fetchCurrentWeather("Los Ang", Locale.getDefault().language)
    }

    private fun isFetcheCurrentNeeded(lastFetchTime: ZonedDateTime): Boolean{
